...
describe('places helpers', () => {
  ...
  it('displayEndpoint appends member codes for a metro group name', () => {
    expect(displayEndpoint('Dallas')).toBe('Dallas (DFW/DAL)')
    expect(displayEndpoint('AUS')).toBe('AUS') // plain code unchanged
  })
