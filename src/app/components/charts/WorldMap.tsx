...
function routeKey(k: RouteKey): string {
  // Unordered pair: sort so A→B and B→A merge
...
export function WorldMap({ flights, accent }: { flights: EnrichedFlight[]; accent: string }) {
  const { arcs, dots } = useMemo(() => {
    // Aggregate routes from resolved non-local flights
    ...
    const maxCount = Math.max(...Array.from(routeMap.values()).map((r) => r.count), 1)
    ...
    for (const [, route] of routeMap) {
      ...
      if (!d) continue
      const t = route.count / maxCount
      arcs.push({
        d,
        strokeWidth: 0.8 + t * 2.2,
        opacity: 0.45 + t * 0.55,
      })
    }
    ...
    for (const [, ap] of airportSet) {
      ...
      if (projected) {
        dots.push({ cx: projected[0], cy: projected[1] })
      }
    }
