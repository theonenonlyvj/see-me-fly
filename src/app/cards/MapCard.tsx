...
import { WorldMap, type MapMode } from '../components/charts/WorldMap'
...
const CAPTION: Record<MapMode, string> = {
  routes: 'Great-circle routes; thicker = flown more often.',
  density: 'Heat density — hotter where you fly most.',
  choropleth: 'Countries shaded by how often you’ve visited.',
}
  ...
  return (
    <CardFrame title="Your map" eyebrow="Where you've flown" accent={ACCENT} accentGrad={ACCENT_GRAD} accentSoft={ACCENT_SOFT} icon="🗺️" fullWidth>
      <div style={{ display: 'inline-flex', padding: 3, gap: 2, marginBottom: 14, background: ACCENT_SOFT, borderRadius: 12, border: `1px solid color-mix(in srgb, ${ACCENT} 24%, transparent)` }}>
        {([['routes', 'Routes'], ['density', 'Heat'], ['choropleth', 'Countries']] as [MapMode, string][]).map(([m, label]) => (
      ...
      <p style={{ marginTop: 8, fontSize: 11.5, color: 'var(--ink-2)', fontStyle: 'italic' }}>{CAPTION[mode]}</p>
    </CardFrame>
  )
}
