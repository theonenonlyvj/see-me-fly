...
2. ~~Drop the state split~~ — **REVERSED 2026-06-26: KEEP the US / India / Mexico state split** (as originally spec'd). It's already implemented (`CountriesCard.tsx` + engine `byCountry` `regions`). No change needed — leave the split in. (Polish only if desired: make the state sub-breakdown render nicely / expandable.)
