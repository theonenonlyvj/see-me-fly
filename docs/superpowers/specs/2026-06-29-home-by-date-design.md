...
- **`geoExtremes` (`stats.ts:444-448`) — compute PER HOME BASE, then rank.** Instead of one mixed global extreme, **partition flights by their era-home** — merging eras that share the same `airportKey`-normalized home set into one **base** (your two Dallas eras → one "Dallas"; Milwaukee, RDU, Madrid, London each their own). Compute a geo-extremes set **per base** with that base's fixed `primary` as the reference. This dissolves the deduped-Set problem (each partition has ONE home, so the existing airport-Set logic is valid and the answer is unique per base) AND yields a richer "how far you reached from each place you've lived" view. Each base yields: farthest flight (great-circle from its `primary`) + N/S/E/W extremes, computed over that base's flights. **Rank bases by farthest** — the card becomes a per-base ranking (the top row IS the all-time farthest, so nothing is lost). Replace the hardcoded DFW `HOME` constant; `!hasHome` falls back to today's single-home behavior. Edges: a base with very few flights still shows its extreme (surface the flight count); local flights (`isLocalFlight`) excluded as today. The `GeoExtremes` card renders the ranking; N/S/E/W can show for the selected/primary base.
